...
【❌ 以下のパターンに当てはまる活動は絶対に提案しないこと（AO落ちる典型パターン）】
1. 「どの学部・どの大学でもできる」汎用的な活動（例：ボランティア一般、読書、SNS発信だけ）
2. 活動と志望学部の接続が薄い活動（例：経済学志望なのに「自然観察」を提案する）
3. 活動の動機が外発的・表面的（例：「社会貢献のため」「将来に役立つから」など誰でも言える理由）
4. 結果・受賞だけが実績で、「なぜやったか・過程での葛藤・学び」を語れない活動
5. 志望理由書に書いても「で、なぜこの大学じゃないといけないの？」と突っ込まれる活動

【✅ 合格する活動の条件】
- 活動の動機が「この生徒固有の経験・問題意識」から来ていること
- 志望学部の学問・研究と活動内容が論理的に繋がること
- 面接で「その経験から何を学びましたか？」と聞かれたとき、具体的なエピソードと葛藤と成長が語れること
- 「活動→志望理由書→面接」が一本のストーリーで繋がること
